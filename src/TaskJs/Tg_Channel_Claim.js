...
const TgChannelClaimBord = ({ onClose, setVisibleClaim, Epic_Claim, userId }) => {
  ...
  return (
    <div className={`Task_Border ${isClosingForAnim ? 'closing' : ''}`} id="ClimeDiv">
      <div className="BackButtom" onClick={(event) => { onClose(event); handleCloseAnim(event); }}> 
      ...
      <button onClick={Tg_Channel_Open_chek}> 
        <img src={Tg} alt='Tg' id='ButtomIMG'/>YOU'RE SUBBED
      </button>
      <div className='forClaim'>
        <img src={Epic_Claim} alt='Epic_Claim_item' id='ClaimIMG'/>
        <button id="ClaimBTN" onClick={() => { handleClaimRewards(); }}>CLAIM</button>
      </div>
    </div>
  );
};
